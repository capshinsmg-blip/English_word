...
- 탭 4개: 홈 / 단어장(공개, 검색 + 상황 그룹 접이식 테마 칩) / 나의 학습 / 설정
  - 테마 구조(data.js): `_RAW_THEMES`(원본 112종, id 목록) → `_pick()`으로 합쳐 `THEME_GROUPS`(14개 상황 그룹 × 묶음 테마 43종) → `THEMES`는 `THEME_GROUPS`에서 평평하게 파생. 단어장은 그룹을 `<details>`로 접었다 펴고, 그 안에 묶음 테마 칩 표시
