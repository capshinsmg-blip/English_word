...
- [x] 단어 데이터 확장 — 현재 8,000개 (원본 테마 204종 → 묶음 테마 99종)
...
- [x] Phase 1 — `words-3.js` id 4001~5000 (B1 보강~B2 진입, 2026-07). 신규 테마 19종 등록
  - 생성 시 교훈: **기존 4,000개가 회화 필수 표현·감정·비즈니스·이디엄을 이미 폭넓게 커버** — 신규 생성분 47%가 중복이라 전량 교체함. 다음 Phase는 생성 전에 반드시 기존 w 목록과 대조 필터를 돌릴 것 (세부 명사·B2/C1 추상어·전문 도메인이 안전지대)
- [x] Phase 2 — `words-4.js` id 5001~6000 (B2 대량: 법·정치·경제·의학·범죄·HR·인지동사·질감·외양·수량·양육, 2026-07). 신규 테마 11종 등록
  - 절차 확립: 후보 word-only 과잉 생성(1,349) → 기존 대조 필터(생존 1,187) → 1,000개 id 배정 → 엔트리 작성 → 조립 검증(중복 0). 이 순서를 Phase 3+에서도 유지할 것
- [x] Phase 3 — `words-5.js` id 6001~7000 (B2 잔여~C1 진입: 과학·우주·지리·종교철학·군사·문학·성격 C1·감정 C1·도시·농업·학문통계·C1 동사, 2026-07). 신규 테마 12종 등록 (원본 192종 → 묶음 87종)
  - 스왑 시 교훈: 필터가 만든 어색한 합성 후보("plaza square" 등)를 자연어로 스왑할 때 **대체 단어도 반드시 기존 전체 + 배정표 잔여분 + 이미 쓴 파트와 대조**할 것 (이번에 instinct/devotion/intuition 3건이 기존과, plow/orchard 2건이 배정표 뒷블록과 충돌해 재교체). 조립 검증이 전부 잡아주므로 마지막 방어선은 유지됨
- [x] Phase 4 — `words-6.js` id 7001~8000 (C1 위주: 예술·공연·언론·환경·금융·역사·항해·심리인지·수공예·C1 추상명사·C1 형용사·C1 담화, 2026-07). 신규 테마 12종 등록 (원본 204종 → 묶음 99종)
  - 절차 개선: 스왑 대체 단어를 넣기 **전에** 기존 전체+작성분 grep 확인 → 조립 첫 시도에 중복 0 달성. 어색한 합성 후보(배정표의 "X era"류)는 발견 즉시 스왑
- [ ] Phase 5~6 — 새 단어 2,000개 생성 (세션당 ~1,000개, id 8001~10000, 난이도별 rank 지정 + 테마 등록)
  - 우선순위: C1~C1+ 잔여 도메인. 각 배치마다 `_RAW_THEMES`에 새 테마(또는 기존 테마에 id 추가) 등록 필수
